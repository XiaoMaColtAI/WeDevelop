...
const promptTextareaRef = ref<any>()
...
// 处理键盘事件：Enter 发送，Shift+Enter 或 Ctrl+Enter 换行
const handleKeyDown = (e: KeyboardEvent) => {
  if (e.key === 'Enter' && !e.shiftKey && !e.ctrlKey) {
    e.preventDefault()
    createApp()
  }
  // Shift+Enter 或 Ctrl+Enter 允许默认行为（换行）
}

// 自动调整 textarea 高度
const adjustTextareaHeight = () => {
  nextTick(() => {
    if (!promptTextareaRef.value) return

    // Ant Design Vue 的 textarea 组件结构
    let textarea: HTMLTextAreaElement | null = null
    const componentEl = promptTextareaRef.value.$el || promptTextareaRef.value
    if (componentEl) {
      textarea = componentEl.querySelector?.('textarea') || (componentEl.tagName === 'TEXTAREA' ? componentEl : null)
    }

    if (textarea) {
      textarea.style.height = 'auto'
      const scrollHeight = textarea.scrollHeight
      const minHeight = 60 // 最小高度，约等于 4 行
      const maxHeight = 300 // 最大高度，约等于 15 行
  ...
  <div id="homePage">
    <div class="container">
      ...
      <div class="hero-section">
        ...
        <p class="hero-description">一句话，创建&部署网站</p>
      </div>
      ...
      <div class="quick-actions">
        <a-button
          ...
          @click="
            setPrompt(
              '创建一个个人博客网站，需要有文章列表、详情页、分类标签、搜索功能和个人简介页面。文章支持Markdown格式，首页展示十篇最新文章。',
            )
          "
        >个人博客</a-button
        >
        <a-button
          ...
          @click="
            setPrompt(
              '设计一个苹果官网，需要有公司介绍、产品服务展示、新闻资讯等页面。采用商务风格的设计，需要有轮播图、产品展示卡片，支持多语言切换和在线客服功能。',
            )
          "
        >苹果官网</a-button
        >
        <a-button
          ...
          @click="
            setPrompt(
              '设计一个黄山旅游宣传网页，需要有景点介绍、旅游计划推荐、旅游路线推荐等。经典介绍要详细，要有景点位置显示（采用一个地图的样式）。整体风格使用更适合景点特点的样式和风格。',
            )
          "
        >旅游宣传网页</a-button
        >
        <a-button
          ...
          @click="
            setPrompt(
              '制作一个作品展示网站，适合设计师、摄影师、艺术家等创作者。包含作品画廊、项目详情页、个人简历、联系方式等模块。采用瀑布流或网格布局展示作品，支持图片放大预览和作品分类筛选。',
            )
          "
        >作品展示网站</a-button
        >
